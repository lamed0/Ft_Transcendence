...
function checkEmail(email) {
	...
	// If the user typed "hello.com" (missing the @)
...
function checkP(password, cpassword) {
    console.log("Password entered:", password);
    console.log("Confirm Password entered:", cpassword);
    if (password !== cpassword || cpassword.length === 0 || password.length === 0) {
...
export default function AppSignUp() {
	...
	const checkEmail = async (email) => {
			...
			const isValid = emailPattern.test(email); 
			if (isValid) {
				// const response = await fetch('http://localhost:5173/api/sendVerificationCode', {
				// 	method: 'POST',
				...
				// 	setStep(2);
				// } else {
				// 	window.alert("Error sending verification code. Please try again.");
				// }
	...
	const checkUsername =  async (username) => {
		...
		if (username.length < 3 || username.length > 20) {
			window.alert("Username must be between 3 and 20 characters.");
		}
		else {
			setStep(4);
			window.alert("Username is valid. Sign-up complete!");
			const formData = {
    ...
    // 1. Logic to send data to your database/API
    console.log("Form Submitted!", { email, password, userName });

    // 2. Provide user feedback (e.g., redirect or show a success message)
				...
				<form action="../" onSubmit={handleSubmit}>
					...
					{step === 4 && (
						<>
							<div id="step3" className="mt-10">
								...
								<p className="font-pixel pb-2">didn't receive it? <button className="text-cyan-400" >resend</button></p>
							</div>
							<a href="../../Home.html" className=" mt-[80] p-2 bg-[#FF00E5] text-white px-8 py-4 rounded-full font-['Press_Start_2P'] text-2xl tracking-widest border-b-8 border-r-8 border-[#B500A2] active:border-b-0 active:border-r-0 active:translate-y-2 active:translate-x-2 transition-all duration-75 shadow-[0_0_20px_rgba(255,0,229,0.5)] hover:shadow-[0_0_35px_rgba(255,0,229,0.8)]">Verify</a>
						</>
					)}
					{/* entering password */}
					{step === 2  && (
						<>
							<div id="step2" className="mt-5 ">
								...
								<input type="password" id="password" name="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password..."  className="text-[20px] font-pixel bg-black border-2 border-[#00FFFF] rounded-[40px] p-2 w-full"/><br/><br/>
					...
					{step === 3 && (
						<>
							...
							<button id="usernameButton" type="submit" onClick={() => checkUsername(userName)} className=" mt-[80] p-2 bg-[#FF00E5] text-white px-8 py-4 rounded-full font-['Press_Start_2P'] text-2xl tracking-widest border-b-8 border-r-8 border-[#B500A2] active:border-b-0 active:border-r-0 active:translate-y-2 active:translate-x-2 transition-all duration-75 shadow-[0_0_20px_rgba(255,0,229,0.5)] hover:shadow-[0_0_35px_rgba(255,0,229,0.8)]">Finish</button>
