...
import { setupNetwork, waiting_player, score , wsClient, front_side} from './client_game.js';
...
class renderUI{
    ...
    createHUD(name: string, position: Vector3){
        ...
        const createPlayerRow = (name :string, color : string, setScore : number, gameScore : number) => {
            ...
            grid.addColumnDefinition(0.45); // Name
            grid.addColumnDefinition(0.15); // Set Score
            grid.addColumnDefinition(0.15); // Game Score
            grid.addColumnDefinition(0.15); // Game Score
        ...
        const player1Row = createPlayerRow(Myid, "HotPink", 0, 2);
        const player2Row = createPlayerRow(Opponent, "darkblue",0, 1);
        ...
        // Add subtle floating animation to HUD
        this.addHUDAnimation(mainStack);
    }
    
    private addHUDAnimation(stack: GUI.StackPanel) {
        const startTop = "15px";
        const offsetAmount = 3; // pixels to move up/down
        let offsetDirection = 1; // 1 for down, -1 for up
        let elapsedTime = 0;
        ...
        const animate = () => {
            ...
            // Smooth sine wave for gentle floating motion
            const offset = Math.sin(cycleProgress * Math.PI * 2) * offsetAmount;
            stack.top = (15 + offset) + "px";
...
export class App {
    ...
    private async _main():Promise<void>{
        ...
        const opponentObj = matchData.opponent; 
        Opponent = opponentObj ? opponentObj.username : null;
        // Find the player who is NOT the opponent (the current user)
        Myid = players.find(p => p.id !== opponentId).username;
        try {
